...
describe('Message bus', () => {
  ...
  context('while posting', () => {
    ...
    it('should reject if no handler and configured to ensure at least one', () => {
      const bus = create({ensureAtLeastOneHandler: true});
    ...
    it('should return undefined when no handler and bus handlers are exclusive', () => {
      const bus = create({exclusiveHandlers: true});
  ...
  context('having some before handle decorators ', () => {
    it('should execute them before message handling', () => {
      const beforeHandle = [
        withPayload(m => ({order: `${m.payload.order}|first|`})),
        withPayload(m => ({order: `${m.payload.order}|second|`}))
      ];
      const bus = create({beforeHandle});
      let postedMessage;
      bus.register('MyMessage', message => {
    ...
    it('should execute them serially though they are async', () => {
      const beforeHandle = [
        wrap(withPayload(m => ({order: `${m.payload.order}|first|`}))),
        wrap(withPayload(m => ({order: `${m.payload.order}|second|`})))
      ];
      const bus = create({beforeHandle});
      let postedMessage;
      bus.register('MyMessage', message => {
      ...
      return post.then(() => {
        const expected = withPayload(() => ({order: '|initial||first||second|'}))(message);
  ...
  function create(creation) {
    return createMessageBus(Object.assign({log: () => undefined}, creation));
  }

  function withPayload(func) {
    return message => {
