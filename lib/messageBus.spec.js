...
const {wrapToPromise} = require('@arpinum/promising');
...
describe('Message bus', () => {
  ...
  context('while posting', () => {
    ...
    it('wont post to the wrong handler', () => {
      ...
      bus.register('MyRightMessage', () => {
        return wrapToPromise(() => posts.push('first handler'));
      });
      bus.register('MyWrongMessage', () => {
        return wrapToPromise(() => posts.push('second handler'));
      });
  ...
  context('having some before handle decorators ', () => {
    ...
    it('should execute them serially though they are async', () => {
      let beforeHandle = [
        message => wrapToPromise(() => message.updatePayload({order: `${message.payload.order}|first|`})),
        message => wrapToPromise(() => message.updatePayload({order: `${message.payload.order}|second|`}))
      ];
      let bus = createMesssageBus({beforeHandle});
      let postedMessage;
      bus.register('MyMessage', message => {
