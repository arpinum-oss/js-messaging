...
describe('Message bus', () => {
  ...
  context('having some before handle decorators ', () => {
    it('should execute them before message handling', () => {
      const beforeHandle = [
        m => Object.assign({}, m, {payload: {order: `${m.payload.order}|first|`}}),
        m => Object.assign({}, m, {payload: {order: `${m.payload.order}|second|`}})
      ];
    ...
    it('should execute them serially though they are async', () => {
      ...
      return post.then(() => {
        const expected = {type: 'MyMessage', payload: {order: '|initial||first||second|'}};
